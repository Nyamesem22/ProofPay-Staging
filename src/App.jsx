...
import {
  ArrowLeft, ArrowRight, ArrowCounterClockwise, Bank, Bell,
...
function Sidebar({ view, setView, onLogout }) {
  return <aside className="sidebar">
    <div className="brand-lockup"><img className="brand-logo-full" src="/assets/proofpay-horizontal-dark.png" alt="ProofPay - Pay Safe. Pay Smart." /><img className="brand-logo-mark" src="/icons/proofpay-192.png" alt="ProofPay" /></div>
    <nav className="side-nav" aria-label="Main navigation">{navItems.map(({ id, label, icon: Icon }) => <motion.button key={id} className={`nav-item ${id === view ? "nav-item--active" : ""}`} onClick={() => setView(id)} whileHover={{ x: 3 }} whileTap={{ scale: 0.98 }}><Icon size={24} /><span>{label}</span></motion.button>)}</nav>
    <div className="sidebar-spacer" />
    <div className="sidebar-utility"><button onClick={onLogout}><SignOut size={24}/><span>Sign out</span></button></div>
  </aside>;
}
...
function InstallAppButton(){
  ...
  return <button className="top-link install-app-button" onClick={install}><DownloadSimple size={21}/><span>Install app</span></button>;
}

function Header({ openModal, onLogout, user }) {
  const firstName=user?.fullName?.trim().split(/\s+/)[0]||"Kojo";
  return <header className="topbar simple-topbar"><div><p>Welcome back</p><h1>Hello, {firstName}</h1></div><div className="topbar-actions">
    <InstallAppButton/>
    <button className="top-link" onClick={() => openModal("language")} aria-label="Language"><Globe size={22} /><span>English</span><CaretDown size={16} /></button>
    <button className="top-link" onClick={() => openModal("help")} aria-label="Help"><Question size={22} /><span>Help</span></button>
...
function SimpleHomeView({stage,openModal,setView}){
  const status=stage==="released"?"Released":stage==="disputed"?"On hold":stage==="delivered"?"Ready to release":"Protected";
  const actions=[
    {label:"Pay",note:"Protect a payment",icon:Plus,tone:"blue",run:()=>openModal("create")},
    {label:"Track",note:"Check payment status",icon:MagnifyingGlass,tone:"green",run:()=>openModal("track")},
    {label:"Report",note:"Get help with a payment",icon:Flag,tone:"amber",run:()=>openModal("dispute")},
    {label:"Use *719#",note:"No internet needed",icon:Phone,tone:"navy",run:()=>openModal("ussd")},
  ];
  return <section className="simple-home">
    <div className="simple-balance-card">
      <div><span>Money protected</span><h2>GHS 300.00</h2><small><ShieldCheck size={16} weight="fill"/> Safe with a licensed partner</small></div>
      <button onClick={()=>openModal("track")}><span>{status}</span><ArrowRight size={18}/></button>
    </div>
    <div className="simple-actions" aria-label="Quick actions">{actions.map(({label,note,icon:Icon,tone,run})=><button key={label} onClick={run}><span className={`simple-action-icon simple-action-icon--${tone}`}><Icon size={25} weight="fill"/></span><b>{label}</b><small>{note}</small></button>)}</div>
    <div className="simple-section-heading"><div><h2>Current payment</h2><p>Blender · Ama Store</p></div><button onClick={()=>openModal("track")}>View details <ArrowRight size={16}/></button></div>
    <button className="simple-current-payment" onClick={()=>openModal("track")}>
      <img src="/assets/blender.png" alt="Blender"/>
      <span><b>GHS 300.00</b><small>Delivery due today</small></span>
      <span className={`simple-status simple-status--${status.toLowerCase().replaceAll(" ","-")}`}>{status}</span>
      <ArrowRight size={18}/>
    </button>
    <div className="simple-section-heading simple-section-heading--activity"><div><h2>Recent activity</h2><p>Your latest protected payments</p></div><button onClick={()=>setView("transactions")}>See all <ArrowRight size={16}/></button></div>
    <div className="simple-activity-list">{transactions.slice(0,3).map((item,index)=>{const Icon=item.icon;const itemStatus=index===0?status:item.status;return <button key={item.name} onClick={()=>index===0?openModal("track"):setView("transactions")}><span className={`product-icon product-icon--${item.tone}`}><Icon size={22}/></span><span><b>{item.name}</b><small>{item.seller} · {item.date}</small></span><span><b>{item.amount}</b><small>{itemStatus}</small></span><ArrowRight size={16}/></button>})}</div>
    <button className="simple-safety-note" onClick={()=>setView("help")}><LockKey size={20}/><span><b>Keep your MoMo PIN private</b><small>ProofPay will never ask for it.</small></span><ArrowRight size={17}/></button>
  </section>
}
...
function TransactionsView({stage,openModal,onSelect,activityRows=syncedActivityRows}){
  ...
  return <motion.section className="simple-page simple-activity-page" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}>
    <div className="simple-page-title"><div><h2>Activity</h2><p>{totals.count} payments</p></div><button onClick={()=>openModal("create")}><Plus size={19}/> New payment</button></div>
    <div className="simple-money-summary"><button onClick={()=>setStatusFilter("Protected")}><span>Protected</span><b>{money(totals.protected)}</b></button><button onClick={()=>setStatusFilter("Released")}><span>Released</span><b>{money(totals.released)}</b></button><button onClick={()=>setStatusFilter("Refunded")}><span>Refunded</span><b>{money(totals.refunded)}</b></button></div>
    <div className="simple-filter-bar"><label><MagnifyingGlass size={19}/><input value={query} onChange={e=>setQuery(e.target.value)} aria-label="Search activity" placeholder="Search payments"/></label><select aria-label="Filter activity" value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}><option>All</option><option>Protected</option><option>Ready to release</option><option>Released</option><option>Refunded</option><option>On hold</option><option>Cancelled</option></select><button onClick={exportStatement} aria-label="Download statement"><DownloadSimple size={20}/></button></div>
    <div className="simple-payment-list">{visible.length?visible.map(item=><button key={item.ref} onClick={()=>item.ref==="PP-260822-9X7L"?openModal("track"):onSelect(item)}><span className={`history-product-mark history-product-mark--${item.status.toLowerCase().replaceAll(" ","-")}`}><Receipt size={20} weight="fill"/></span><span><b>{item.item}</b><small>{item.seller} · {item.date}</small></span><span><b>{money(item.amount)}</b><small>{item.status}</small></span><ArrowRight size={17}/></button>):<div className="simple-empty"><MagnifyingGlass size={30}/><b>No payments found</b><button onClick={()=>{setQuery("");setStatusFilter("All")}}>Clear filters</button></div>}</div>
    <div className="simple-record-note"><LockKey size={17}/> Payment records cannot be changed.</div>
  </motion.section>
}
...
function MessagesView({notify}){
  ...
  return <motion.section className="simple-page simple-messages-page" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}><div className="simple-page-title"><div><h2>Messages</h2><p>{customerConversations.reduce((sum,item)=>sum+item.unread,0)} unread</p></div><button onClick={()=>setSelected(customerConversations[1])}><Headset size={19}/> Support</button></div><div className="simple-messages-layout"><aside className="simple-conversation-list">{customerConversations.map(item=><button key={item.id} className={selected.id===item.id?"is-active":""} onClick={()=>setSelected(item)}><span className={`conversation-avatar conversation-avatar--${item.tone}`}>{item.initials}</span><span><b>{item.name}</b><small>{item.preview}</small></span><span><small>{item.time}</small>{item.unread>0&&<i>{item.unread}</i>}</span></button>)}</aside><section className="chat-panel simple-chat-panel"><header><span className={`conversation-avatar conversation-avatar--${selected.tone}`}>{selected.initials}</span><div><b>{selected.name}</b><small>Verified · {selected.role}</small></div><button aria-label={`Call ${selected.name}`} onClick={()=>notify(`Call request prepared for ${selected.name}.`)}><Phone size={20}/></button></header><div className="simple-chat-safety"><LockKey size={15}/> Never share your MoMo PIN.</div><div className="chat-thread">{(messages[selected.id]||[]).map((message,index)=><div className={message.mine?"is-mine":""} key={`${message.time}-${index}`}><span>{message.text}</span><small>{message.time}</small></div>)}</div><footer><button aria-label="Attach evidence" onClick={()=>notify("Evidence attachment picker opened.")}><UploadSimple size={21}/></button><label><input value={draft} onChange={e=>setDraft(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Write a message"/></label><button className="simple-send-button" aria-label="Send message" onClick={send}><PaperPlaneTilt size={20} weight="fill"/></button></footer></section></div></motion.section>
}
...
function SettingsView({notify}){
  const [tab,setTab]=useState("Profile"),[saved,setSaved]=useState(false);const tabs=["Profile","Security","Payment methods","Notifications"];
  const save=()=>{setSaved(true);notify("Settings saved successfully.");window.setTimeout(()=>setSaved(false),2200)};
  return <motion.section className="simple-page simple-settings-page" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}><div className="simple-page-title"><div><h2>Settings</h2><p>Account and preferences</p></div><StatusBadge saved={saved}/></div><div className="simple-settings-layout"><nav>{tabs.map(item=><button key={item} className={tab===item?"is-active":""} onClick={()=>setTab(item)}><span className={`simple-setting-icon simple-setting-icon--${item.toLowerCase().replace(" ","-")}`}>{item==="Profile"?<User size={21}/>:item==="Security"?<ShieldCheck size={21}/>:item==="Payment methods"?<Wallet size={21}/>:<Bell size={21}/>}</span><span><b>{item}</b><small>{item==="Profile"?"Name, phone and language":item==="Security"?"PIN and trusted devices":item==="Payment methods"?"Wallets and bank accounts":"Alerts and reminders"}</small></span><ArrowRight size={16}/></button>)}</nav><section className="simple-settings-panel">{tab==="Profile"&&<><header><h3>Profile</h3></header><div className="profile-summary"><span className="avatar avatar--blue">KM</span><div><b>Kojo Mensah</b><small>055 123 4567</small></div><span><SealCheck size={17} weight="fill"/> Verified</span></div><div className="simple-settings-form"><label>Preferred name<input defaultValue="Kojo"/></label><label>Email<input defaultValue="kojo.mensah@example.com"/></label><label>Language<select><option>English</option><option>Twi</option><option>Ga</option><option>Ewe</option></select></label></div></>}{tab==="Security"&&<SettingsSecurity/>}{tab==="Payment methods"&&<PaymentMethods/>}{tab==="Notifications"&&<SettingsToggles title="Notifications" items={["Payment updates","Delivery reminders","Dispute messages","Security alerts"]}/>}<footer><AppButton icon={saved?CheckCircle:SealCheck} onClick={save}>{saved?"Saved":"Save"}</AppButton></footer></section></div></motion.section>
}
...
function SettingsSecurity(){return <><header><h3>Security</h3></header><div className="security-settings simple-security-list"><article><ShieldCheck size={23} weight="fill"/><div><b>Phone</b><span>055 *** 4567</span></div><em>Verified</em></article><article><IdentificationCard size={23}/><div><b>Identity</b><span>Ghana Card</span></div><em>Verified</em></article><article><LockKey size={23}/><div><b>Confirmation code</b><span>Protects release actions</span></div><button>Change</button></article><article><DeviceMobile size={23}/><div><b>Trusted devices</b><span>1 device</span></div><button>View</button></article></div><div className="settings-warning"><WarningCircle size={20}/><span><b>Never share your MoMo PIN.</b></span></div></>}
function PaymentMethods(){return <><header><h3>Payment methods</h3></header><div className="payment-method-list simple-method-list"><article><span className="method-logo method-logo--momo">MoMo</span><div><b>MTN MoMo</b><span>055 *** 4567</span></div><em>Primary</em><button>Manage</button></article><article><span className="method-logo">BANK</span><div><b>Bank account</b><span>Not connected</span></div><button>Connect</button></article><article><span className="method-logo">VISA</span><div><b>Visa card</b><span>Not connected</span></div><button>Connect</button></article></div></>}
function SettingsToggles({title,items}){return <><header><h3>{title}</h3></header><div className="toggle-list simple-toggle-list">{items.map(item=><label key={item}><span><b>{item}</b></span><input type="checkbox" defaultChecked/><i/></label>)}</div></>}
...
const wizardSteps=["Recipient","Details","Protection","Review"];
function CreatePaymentModal({onClose,onCreated,onTrack,embedded=false}){
  ...
  const [releaseConfirmed,setReleaseConfirmed]=useState(true);
  ...
  const verifyParty=()=>{setVerifying(true);window.setTimeout(()=>{setVerifying(false);setPartyVerified(true);setPartiesConfirmed(true)},700)};
  return <ModalShell title={complete?"Payment protected":"Create protected payment"} subtitle={complete?"Your agreement is active and the seller can now deliver.":"Set clear terms before money moves."} onClose={onClose} wide className={embedded?"embedded-flow--wizard":"modal--wizard"} embedded={embedded}>
    ...
    <AnimatePresence mode="wait">{complete?<motion.div className="payment-success-page" key="success" initial={{opacity:0,y:12}} animate={{opacity:1,y:0}}>
      ...
      {step===0&&<div className="simple-pay-step">
        <div className="simple-pay-heading"><span className="simple-action-icon simple-action-icon--blue"><User size={25} weight="fill"/></span><div><h3>Who are you paying?</h3><p>Enter the mobile number. We will confirm the wallet name.</p></div></div>
        <div className="simple-pay-form">
          <label>Mobile number<input autoFocus value={form.seller} onChange={e=>updateParty("seller",e.target.value)} inputMode="tel"/></label>
          <label>Wallet provider<select value={form.provider} onChange={e=>updateParty("provider",e.target.value)}><option>MTN MoMo</option><option>Telecel Cash</option><option>AT Money</option><option>Bank account</option></select></label>
          <label className="form-span">Name<input value={form.sellerName} onChange={e=>updateParty("sellerName",e.target.value)} placeholder="Person or business name"/></label>
        </div>
        {!partyVerified?<AppButton className="verify-party-button" icon={verifying?SpinnerGap:ArrowRight} disabled={verifying||!form.seller||!form.sellerName} onClick={verifyParty}>{verifying?"Checking…":"Confirm recipient"}</AppButton>:<motion.div className="simple-recipient-confirmed" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}><SealCheck size={26} weight="fill"/><span><b>{form.sellerName}</b><small>{form.provider} · 024 *** 6543</small></span><button onClick={()=>{setPartyVerified(false);setPartiesConfirmed(false)}}>Change</button></motion.div>}
        <div className="simple-pay-safety"><LockKey size={19}/><span>Your MoMo PIN stays in your wallet.</span></div>
      </div>}
      {step===1&&<div className="simple-pay-step"><div className="simple-pay-heading"><span className="simple-action-icon simple-action-icon--green"><Receipt size={25} weight="fill"/></span><div><h3>What is the payment for?</h3><p>Only three details are needed.</p></div></div><div className="simple-pay-form"><label>Item or service<input value={form.item} onChange={e=>update("item",e.target.value)}/></label><label>Amount (GHS)<input value={form.amount} onChange={e=>update("amount",e.target.value)} inputMode="decimal"/></label><label className="form-span">Delivery date<input value={form.date} onChange={e=>update("date",e.target.value)}/></label></div><div className="simple-protection-defaults"><ShieldCheck size={21} weight="fill"/><span><b>Protection included</b><small>24-hour inspection · delivery proof · dispute hold</small></span></div></div>}
